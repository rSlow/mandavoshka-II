...
export default class AuthAPI {
    static async login(data: LoginRequest): Promise<AxiosAuthResponse> {
        return await $axiosAPI.post<AccessTokenResponse>("/accounts/login/", data)
    }

    static async register(data: RegisterRequest): Promise<AxiosAuthResponse> {
        return await $axiosAPI.post<AccessTokenResponse>('/accounts/register/', data)
    }

    static async tokenRefresh(): Promise<AxiosAuthResponse> {
        return await  $axiosAPI.post<AccessTokenResponse>("/accounts/token/refresh/")
    }

    static async tokenVerify(): Promise<AxiosResponse<VerifyTokenResponse>> {
        return await $axiosAPI.post<object>("/accounts/token/verify/")
    }
}